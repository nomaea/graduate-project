    ...
    try:
        while True:
            ...
            if time.monotonic() >= next_out:
                print("[BIO][OUT] output tick reached")
                next_out += 1.0 / OUT_HZ

                bpm = ppg.bpm()
                rmssd_raw = ppg.rmssd_30s()

                rr_30s = getattr(ppg, "rr_30s", None)
                if rr_30s is None:
                    rr_count = 0
                    print("[BIO][WARN] ppg.rr_30s not ready; using rr_count=0")
                else:
                    rr_count = len(rr_30s)
                    print(f"[BIO][INFO] rr_count={rr_count}")
                ...
                if payload_queue is not None:
                    try:
                        print("[BIO][QUEUE] before put")

                        if payload_queue.full():
                            dropped = payload_queue.get_nowait()
                            print(f"[BIO][QUEUE] dropped old payload ts={dropped.get('timestamp')}")

                        payload_queue.put_nowait(payload)

                        print(
                            f"[BIO][QUEUE] put success "
                            f"ts={payload.get('timestamp')} "
                            f"bpm={payload.get('bpm')} "
                            f"bio_arousal={payload.get('bio_arousal')} "
                            f"bio_conf={payload.get('bio_conf')} "
                            f"note={payload.get('note')}"
                        )
                    except Exception as e:
                        print(f"[BIO][QUEUE][ERROR] {type(e).__name__}: {e}")
