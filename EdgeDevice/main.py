    ...
    def _drain_bio_queue(self):
        ...
        if latest is not None:
            ...
            print(
                ...
                f"ts={latest.get('timestamp')} "
                f"bpm={latest.get('bpm')} "
                f"rmssd_use={latest.get('rmssd_use')} "
                f"gsr_cur={latest.get('gsr_cur')} "
                f"bio_arousal={latest.get('bio_arousal')} "
                f"bio_conf={latest.get('bio_conf')} "
                f"note={latest.get('note')}"
            )
        else:
            print(f"[DEBUG][BIO_QUEUE] no new payload qsize={self.bio_payload_queue.qsize()}")
