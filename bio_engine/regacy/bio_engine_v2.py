...
# ----------------------------
class PPGProcessor:
    def __init__(self, fs_hz: int, finger_ir_min: int = 50000):
        self.fs = fs_hz
    ...
    def bpm(self):
        if not self.ir:
            return None
        now_t = self.ir[-1][0]
        rrs = [rr for (tt, rr) in self.rr if (now_t - tt) <= 3.0]
        if len(rrs) < 2:
            return None
...
def main():
    ...
    ppg = PPGProcessor(fs_hz=PPG_FS, finger_ir_min=IR_FINGER_MIN)
    ...
    try:
        while True:
            ...
            if gsr_val is not None:
                gsr_buf.append((now_t, int(gsr_val)))
